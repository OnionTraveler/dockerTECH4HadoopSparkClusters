#!/bin/bash
 ...
 [ `docker ps -a | grep 'master' | rev | cut -d ' ' -f 1 | rev` ] && echo "The container 「master」 has existed" || docker run -itd --name master --hostname master -p 50070:50070 -p 8088:8088 -p 8080:8080 -p 4040:4040 oniontraveler/hadoop_container:19.5.4
 [ `docker ps -a | grep 'slaver1' | rev | cut -d ' ' -f 1 | rev` ] && echo "The container 「slaver1」 has existed" || docker run -id --name slaver1 --hostname slaver1 -p 50071:50070 oniontraveler/hadoop_container:19.5.4
