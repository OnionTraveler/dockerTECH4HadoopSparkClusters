#!/bin/bash
 ...
 [ `docker ps -a | grep 'slaver1' | rev | cut -d ' ' -f 1 | rev` ] && echo "The container 「slaver1 」 has existed" || docker run -id --name slaver1 --hostname slaver1 oniontraveler/hadoop_container:19.5.4
